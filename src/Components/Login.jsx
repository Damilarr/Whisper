...
const Login = () => {
  ...
  const googleSignIn = async (e) => {
    ...
    signInWithPopup(auth, provider)
      .then(async (result) => {
        ...
        if (!docSnap.exists()) {
          createFireStoreUser(uid, displayName, email, photoURL);
        }
        redirect(result);
      })
      .catch((error) => {
  ...
  const createFireStoreUser = async (uid, userName, email, photoURL) => {
  ...
  return (
    <section className="bg-gray-900 rounded-lg shadow-md h-fit">
      <div className="container flex items-center justify-center min-h-screen px-6 mx-auto">
        <form className="w-full max-w-md">
          ...
          <h1 className="mt-3 text-2xl font-semibold capitalize sm:text-3xl text-white">
